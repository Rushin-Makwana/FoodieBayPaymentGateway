...
if(!paymentDetails.amount || !paymentDetails.customerEmail || !paymentDetails.customerPhone ) {
  res.status(400).send('Payment failed')        
    ...
    params['CALLBACK_URL'] = "https://foodiebay-payment.herokuapp.com/callback";
     ...
     checksum_lib.genchecksum(params, config.PaytmConfig.key, function (err, checksum) {
       ...
       var post_req = https.request(options, function(post_res) {
         ...
         post_res.on('end', function(){
           ...
           res.redirect(`https://foodiebay.netlify.app/viewBooking?status=${_results.STATUS}&ORDERID=${_results.ORDERID}&date=${_results.TXNDATE}&bank=${_results.BANKNAME}`)
           });
       });
